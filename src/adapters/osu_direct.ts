...
export function osuDirectBeatmapsetFromCheesegullBeatmapset(beatmapset: CheesegullBeatmapset): string {
    ...
    const formattedBeatmaps = difficultySortedBeatmaps.map(beatmap => osuDirectBeatmapFromCheesegullBeatmap(beatmap)).join(",");
...
export function osuDirectBeatmapsetCardFromCheesegullBeatmapset(beatmapset: CheesegullBeatmapset): string {
