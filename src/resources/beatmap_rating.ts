import {Database, BeatmapRating, NewBeatmapRating} from '../database';
...
export class BeatmapRatingRepository {
    ...
    async create(beatmapRating: NewBeatmapRating): Promise<BeatmapRating> {
        const { insertId } = await this.database.insertInto('beatmaps_rating')
    ...
    async getRating(beatmapMd5: string): Promise<BeatmapRatingResult> {
        const ratingResult = await this.database.selectFrom('beatmaps_rating')
    ...
    async fromUserIdAndBeatmapMd5(userId: number, beatmapMd5: string): Promise<BeatmapRating | null> {
        const rating = await this.database.selectFrom("beatmaps_rating")
            .where("beatmap_md5", "=", beatmapMd5)
