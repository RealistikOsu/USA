...
import { RIPPLE_LOVED, RIPPLE_RANKED, RIPPLE_APPROVED, RIPPLE_QUALIFIED, RIPPLE_PENDING } from "../adapters/beatmap";
...
export class BeatmapService {
    ...
    private async discoverNewBeatmapSet(beatmapSetId: number): Promise<Beatmap[]> {
        ...
        for (const existingBeatmap of existingBeatmaps) {
            ...
            if (!found) {
                logger.debug("Beatmap has been unsubmitted. Deleting difficulty.", {
                    beatmapId: existingBeatmap.beatmap_id,
                });
                await this.beatmapRepository.deleteByBeatmapId(existingBeatmap.beatmap_id);
            }
        }
    ...
    private async tryUpdateBeatmap(beatmap: Beatmap): Promise<ErrorOr<Beatmap>> {
        ...
        const timeSinceUpdate = Math.floor(Date.now() / 1000) - beatmap.latest_update;
        ...
        const updatedBeatmap = await this.getBeatmapFromOsuApi(beatmap.beatmap_id);
    ...
    async findByBeatmapMd5(beatmapMd5: string, extra?: ExtraBeatmapData): Promise<ErrorOr<Beatmap>> {
        let beatmap = await this.beatmapRepository.findByMd5(beatmapMd5);

        if (beatmap === null) {
            if (extra === undefined) {
                logger.debug("Fetching a single beatmap from the osu!api by checksum.", {
                ...
                const fileNameBeatmap = await this.beatmapRepository.fromFileName(extra.fileName);
                if (fileNameBeatmap !== null && fileNameBeatmap.latest_update - Math.floor(Date.now() / 1000) < ONE_DAY) {
                    return ServiceError.BEATMAP_UPDATE_REQUIRED;
                }

                const updatedSet = await this.discoverNewBeatmapSet(extra.beatmapSetId);
    ...
    async findByBeatmapId(beatmapId: number, extra?: ExtraBeatmapData): Promise<ErrorOr<Beatmap>> {
        let beatmap = await this.beatmapRepository.findByBeatmapId(beatmapId);

        if (beatmap === null) {
            if (extra === undefined) {
                logger.debug("Fetching a single beatmap from the osu!api by beatmap ID.", {
                    beatmapId: beatmapId,
                })
    ...
    private async getBeatmapFromOsuApiByChecksum(beatmapMd5: string): Promise<Beatmap | null> {
