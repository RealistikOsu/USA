import { RelaxType, sortColumnFromRelaxType } from "../adapters/osu";
import { FetchManyScoresParameters, FindByUserIdParameters, ScoreRepository, ScoreWithRankAndUsername } from "../resources/score";
...
export class LeaderboardService {
    ...
    async fetchByBeatmapMd5(params: FetchLeaderboardParameters): Promise<Leaderboard> {
        ...
        const scores = await this.scoreRepository.fetchManyWithRankAndUsername(fetchManyScoresParameters);
        const personalBest = await this.scoreRepository.findByUserIdWithRankAndUsername(findByUserIdParameters);
        const scoreCount = await this.scoreRepository.fetchScoreCount(fetchManyScoresParameters);
